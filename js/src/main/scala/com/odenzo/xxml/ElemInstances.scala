...
trait ElemInstances {
  ...
  /** Encodes the XML as Text with the implicit charset defaulting to UTF-8 */
  implicit def xxmlEncoder[F[_]](implicit charset: Charset = `UTF-8`): EntityEncoder[F, Elem] =
    EntityEncoder.stringEncoder
      .contramap[Elem] { node => XXMLParser.serialize(node, charset.toString) }
  ...
  /** Handles a message body as XML. This is renamed xxml but you probably are never going to use the name and avoids conflict with
    ...
    *   an XML element
    */
  implicit def xxmlDecoder[F[_]](implicit F: Concurrent[F]): EntityDecoder[F, Elem] = {
